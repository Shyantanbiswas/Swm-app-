...
interface StaffNoticeModalProps {
    message: string;
    onDismiss: () => void;
}

const StaffNoticeModal: React.FC<StaffNoticeModalProps> = ({ message, onDismiss }) => {
    const { t } = useLanguage();
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-[100] p-4 animate-fade-in">
            <div className="bg-card-light dark:bg-card-dark rounded-xl shadow-2xl w-full max-w-md relative transform transition-all animate-scale-in text-center overflow-hidden">
                <div className="bg-gradient-to-r from-primary to-accent p-6 text-white">
                    ...
                    <h3 className="text-2xl font-bold mt-3">{t('aMessageForYou')}</h3>
                </div>
                ...
                <div className="p-6">
                    <p className="text-lg text-text-light dark:text-text-dark whitespace-pre-wrap italic">"{message}"</p>
                </div>
...
export default StaffNoticeModal;

...
const AdminAuthFlow: React.FC<AdminAuthFlowProps> = ({ onBack }) => {
    ...
    const generateAndSendOtpRequest = () => {
        ...
        const messageForAdmin = `[Admin-Only] EcoTrack ADMIN Login Request. Your OTP is: ${newOtp}`;
        const whatsappUrl = `https://wa.me/91${SUPPORT_PHONE_NUMBER}?text=${encodeURIComponent(messageForAdmin)}`;
                ...
                <div className="bg-card-light dark:bg-card-dark p-8 rounded-2xl shadow-xl w-full relative">
                    ...
                    {step === 'login' ? (
                        <form onSubmit={handleSendOtp} className="animate-scale-in">
                            <h2 className="text-2xl font-semibold text-heading-light dark:text-heading-dark mb-2 text-center">Verification</h2>
                            <p className="text-text-light dark:text-text-dark mb-6 text-sm text-center">Enter the administrator mobile number.</p>
                         ...
                         <form onSubmit={handleVerifyOtp} className="animate-scale-in">
                            <h2 className="text-2xl font-semibold text-heading-light dark:text-heading-dark mb-2 text-center">Verify OTP</h2>
                            <p className="text-text-light dark:text-text-dark mb-4 text-sm break-words text-center">
                                OTP request sent for <span className="font-bold">{identifier}</span>.
                                <button type="button" onClick={changeIdentifier} className="text-primary ml-2 text-sm font-semibold">Change</button>
                            </p>

...
import { Shield, KeyRound, ArrowRight, RefreshCw, Smartphone, ArrowLeft, Loader2, User } from 'lucide-react';
...
const StaffAuthFlow: React.FC<StaffAuthFlowProps> = ({ onBack }) => {
    const { t } = useLanguage();
    ...
    const generateAndSendOtpRequest = () => {
        ...
        const messageForAdmin = `[EcoTrack Staff Login] Your login OTP for ${identifier} is: ${newOtp}`;
    ...
    const handleSendOtp = (e: React.FormEvent) => {
        ...
        const staffRoles: UserType['role'][] = ['employee', 'captain', 'sanitaryworker'];
        // Check if a user exists with this identifier and has a staff role, or is the admin override
        const isStaff = users.some(u => u.identifier === identifier && (staffRoles.includes(u.role) || u.identifier === '9635929052'));

        if (!isStaff) {
            setError(`Access Denied. This number is not registered as a staff member.`);
            return;
        }
        setError('');
    ...
    const handleVerifyOtp = async (e: React.FormEvent) => {
        ...
        setError('');
        setIsLoading(true);
        const result = await auth.loginAsStaff(identifier);
        setIsLoading(false);
        if (!result.success) {
            ...
            <div className="w-full max-w-sm mx-auto relative">
                ...
                <div className="text-center mb-8">
                    <div className="flex justify-center items-center gap-3 mb-4">
                        ...
                        <h1 className="text-4xl font-bold text-heading-light dark:text-heading-dark">{t('staffLogin')}</h1>
                    </div>
                </div>

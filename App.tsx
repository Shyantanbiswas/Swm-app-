...
const UserApp: React.FC<{ users: User[] }> = ({ users }) => {
  ...
  const profileActionRequired = isPanchayatMissing || forcePasswordChange;
  ...
  return (
    <div className="w-full max-w-lg mx-auto bg-card-light dark:bg-card-dark shadow-2xl flex flex-col h-screen">
      ...
      <BottomNav currentView={currentView} setCurrentView={setCurrentView} profileActionRequired={profileActionRequired} />
      <button
...
const App: React.FC = () => {
  ...
  const { users } = useData();
  const [loginMode, setLoginMode] = useState<'selector' | 'user' | 'admin' | 'staff'>('selector');
  ...
  if (!isLoggedIn || !user) {
    switch (loginMode) {
        ...
        case 'staff':
            return <StaffAuthFlow onBack={() => setLoginMode('selector')} />;
        case 'selector':
        default:
